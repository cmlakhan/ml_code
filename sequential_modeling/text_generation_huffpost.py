...
news_df[['count_headline']] = news_df.headline_ascii.apply(lambda x: len(x))
...
news_df = news_df[news_df['count_headline'] <= 100]


rnn = RNN(n_letters, 2048, n_letters)
...
for i in range(1,num_epochs+1):
    ...
    subset_df = news_df.sample(frac=frac)
    len_df = subset_df.shape[0]
    for index,row in subset_df.iterrows():
        iter += 1
        category_tensor = categoryTensor(row.category)
        input_line_tensor = inputTensor(row.headline_ascii)
        target_line_tensor = targetTensor(row.headline_ascii)
